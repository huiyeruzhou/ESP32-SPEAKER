...
void micronphone_task(void *pvParameters) {
  ...
  while (1) {
    ...
    /*
       ...
       sizeof(timeout)) < 0) { perror("setsockopt error"); return -1;
            }
            */
    ...
    opus_int16 *in1 = reinterpret_cast<opus_int16 *>(
        malloc(sizeof(opus_int16) * encodedatasize));
    // char * i2s_rx = in1;
    ...
    while (1) {
      ...
      ESP_ERROR_CHECK(
          i2s_read(I2S_NUM_0, in1, encodedatasize, &BytesRead, portMAX_DELAY));
...
void i2s_config_proc() {
  ...
  i2s_config_t i2s_config = {
      ...
      .channel_format =
          I2S_CHANNEL_FMT_RIGHT_LEFT,  // I2S_CHANNEL_FMT_ONLY_RIGHT,
  ...
  // ESP_ERROR_CHECK(i2s_set_clk(I2S_NUM_0, RATE,
  // I2S_BITS_PER_SAMPLE_16BIT,I2S_CHANNEL_STEREO));// I2S_CHANNEL_MONO));
...
OpusEncoder *encoder_init(opus_int32 sampling_rate, int channels,
  ...
  if (enc_err == OPUS_ALLOC_FAIL) {
    printf("OPUS_ALLOC_FAIL error\n");
  } else if (enc_err == OPUS_BAD_ARG) {
    printf("OPUS_BAD_ARG error\n");
  } else if (enc_err == OPUS_BUFFER_TOO_SMALL) {
    printf("OPUS_BUFFER_TOO_SMALL error\n");
  } else if (enc_err == OPUS_INTERNAL_ERROR) {
    printf("OPUS_INTERNAL_ERROR error\n");
  } else if (enc_err == OPUS_INVALID_PACKET) {
    printf("OPUS_INVALID_PACKET error\n");
  } else if (enc_err == OPUS_INVALID_STATE) {
    printf("OPUS_INVALID_STATE error\n");
  } else if (enc_err == OPUS_UNIMPLEMENTED) {
    printf("OPUS_UNIMPLEMENTED error\n");
  }
  ...
  // 被编码信号的深度，是一个提示，低于该数量的信号包含可忽略的量化或其他噪声，帮助编码器识别静音
